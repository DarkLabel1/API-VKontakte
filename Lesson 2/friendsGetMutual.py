...
#Делаем запрос к серверу с такиами параметрами:
#source_uid - 1 пользователь
#target_uid - 2 пользователь
...
#Сделаем так чтобы были понятны имена пользователей а не их id
users_id = ''

for item in r:
    users_id += str(item) + ","

#Про запросы users.get будет рассказано немного позже
